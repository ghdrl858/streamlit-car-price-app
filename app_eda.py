...
def run_eda() :
    ...
    # 라디오버튼을 이용해서, 데이터프레임과 통계치를 선택해서 보여줄 수 있도록 만든다.
    ...
    # 유저가 선택한 컬럼들만 pairplot을 그리고 그 아래 상관계수를 보여준다.
    ...
    if len(selected_list) > 1 :
        ...
        # heatmap 그래프
        fig2 = plt.figure()
        sb.heatmap(data = car_df[selected_list].corr(), annot = True, fmt = '.2f', vmin = -1, 
        vmax = 1, cmap = 'coolwarm', linewidths = 0.5)
        st.pyplot(fig2)
